  ...
  return (
    <div className='mt-10'>
      ...
      <div className='flex gap-x-5 items-center mt-5'>
        <Image src={author.avatar_urls?.['96']} alt='author' width={100} height={100} className='rounded-full' />
        <div className='flex flex-col gap-2 items-start justify-center'>
          <h6 className='text-primary font-bold'>{author.name}</h6>
          <p className='text-gray-600 dark:text-gray-300 font-mono text-sm'>{author.description?.replaceAll('&amp;', '&')}</p>
          <Link href={author.link} target='_blank' rel='noreferrer' className='text-primary font-mono'>
            @{author.slug}
          </Link>
        </div>
      </div>
