...
const BlogFilter = ({ categories, posts }: BlogFilterProps) => {
  ...
  useEffect(() => {
    const results = posts.filter(
      (post) =>
        ...
        search
          .toLowerCase()
          .split(' ')
          .every((category) => post.yoast_head_json.schema?.['@graph']?.[0].articleSection?.includes(category))
    );
    ...
    if (sortOrder.id === 'date') {
      ...
      results.sort((a, b) => b?.title.rendered.localeCompare(a?.title.rendered));
    ...
    <div className='mt-2 flex flex-wrap items-baseline gap-2'>
      ...
      <ul className='mt-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-3'>
        {filteredPosts.length ? (
          filteredPosts.map((post: BlogType) => <BlogCard key={post.slug} post={post} />)
        ) : (
          <ContentPlaceholder />
        )}
      </ul>
