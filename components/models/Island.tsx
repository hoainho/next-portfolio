...
export function Island() {
  ...
  const { mixer } = useAnimations(animations, islandRef);
  ...
  const handlePointerMove = (event: any) => {
    ...
    if (isRotating && islandRef.current) {
      ...
      islandRef.current.rotation.y = Math.max(
        -0.8,
        Math.min(0.8, newRotationY)
      );
  ...
  const handleTouchMove = (event: any) => {
    ...
    if (isRotating && islandRef.current) {
      ...
      const delta = (clientX - lastX.current) / viewport.width;
      if (
        islandRef.current.rotation.y < -0.8 ||
        islandRef.current.rotation.y > 0.8
      ) {
        setIsRotating(false);
  ...
  useFrame(() => {
    if (islandRef.current) {
      if (!isRotating) {
        ...
        islandRef.current.rotation.y = Math.max(
          -0.8,
          Math.min(0.8, islandRef.current.rotation.y + rotationSpeed.current)
        );
      } else {
