...
const BlogCategory = async ({ params }: Props) => {
  ...
  const category = categoriesFilter.find((category: PostCategory) => category.slug === params.slug);

  const breadcrumb = [
    { label: 'Blog', href: '/blog' },
    {
      label: category?.name || params.slug.charAt(0).toUpperCase() + params.slug.slice(1),
  ...
  const postsByCategoryID = await client.query({
    ...
    context: {
      fetchOptions: {
        next: { revalidate: +(process.env.NEXT_PUBLIC_REVALIDATE_POSTS ?? 600) }, // 10 minutes
      },
    },
  });
  ...
  const featuredPost: PostItem | undefined = postsByCategory.find((post: PostItem) =>
            ...
            <div className='flex flex-col gap-y-5'>
              <div className='w-full'>
                <div className='flex items-center justify-between py-5 border-b border-gray-border mb-5'>
                  <h2 className='text-2xl font-bold text-default'>Featured Posts</h2>
                </div>
