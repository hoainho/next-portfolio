...
import { GET_CATEGORIES_QUERY, GET_POSTS_BY_CATEGORY_AND_AUTHOR_QUERY } from '@/graphql/queries/post.query';
...
export default async function BlogLayout({ children }: Props) {
  ...
  const categoriesReq = categoriesFilter.filter((category) => !category.parent).map(async (category) => {
