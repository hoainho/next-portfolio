...
export async function GET(req: NextRequest) {
  try {
    ...
    if (tag) {
      ...
      console.log(`[${getTimestamp()}] Processing ${tags.length} tags for revalidation`);
      ...
      for (const t of tags) {
        try {
          ...
          console.error(`[${getTimestamp()}] Failed to revalidate tag: ${t}`, e);
        }
      }
    }
    ...
    if (path) {
      ...
      console.log(`[${getTimestamp()}] Processing ${paths.length} paths for revalidation`);
      ...
      for (const p of paths) {
        try {
          ...
          console.error(`[${getTimestamp()}] Failed to revalidate path: ${p}`, e);
        }
      }
    }
